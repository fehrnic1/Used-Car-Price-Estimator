...
- Key EDA findings: Price distribution is strongly right-skewed — median $31,000 vs mean $44,553 — with luxury and exotic vehicles forming a long tail; 75 records above $200,000 were removed as outliers. `engine_hp` is missing for 808 of 4,009 rows (≈20%) because it must be parsed from a free-text engine description string and is absent for non-standard entries. `clean_title` has the highest missingness (596 rows, ≈15%) and was encoded as 0 when absent. See *Exploratory Data Analysis* in [`01_ML_Numeric/used_car_price_prediction.ipynb`](01_ML_Numeric/used_car_price_prediction.ipynb#2-exploratory-data-analysis)
...
- Final results: Best model — Random Forest (Iteration 3, n=200). Test R²: 0.802, Test RMSE: $12,081. Note: Gradient Boosting achieved a slightly better test R² (0.841) and RMSE ($10,816) but the Random Forest was selected as the saved model. RF was preferred for deployment because its trees are evaluated in parallel, giving lower prediction latency than GB's sequential boosting — important for real-time inference in the Gradio app. The cross-validation RMSE difference is also small ($12,178 RF vs $11,658 GB).
...
| 2 | [Car Damage Severity Dataset](https://www.kaggle.com/datasets/prajwalbhamere/car-damage-severity-dataset) | Images (3 classes: minor / moderate / severe) | 1,383 train / 248 val images (Kaggle split); re-split 70/15/15 → 968 train / 207 val / 208 test | Damage assessment → output `condition_score` |
